...
RSpec.describe "Experiment Index Page" do
  ...
  describe 'User story 3' do
    it 'lists all and sorts the experiments by those longer than 6 months by name length descending' do
      visit experiments_path

      within "#sorted-experiments" do
        expect(@double_helix.name).to appear_before(@laws_of_motion.name)
        expect(@laws_of_motion.name).to_not appear_before(@double_helix.name)
